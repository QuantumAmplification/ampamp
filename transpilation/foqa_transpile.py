...
Mathematical Standing Notation (aligned with final.tex and 3.5_FOAA.py):
  ...
  Wave-division operator V_n (acts on ancilla, controlled on index=|0>):
    V_n = [[cos(alpha_n), -sin(alpha_n)],    (a 2x2 rotation matrix)
           [sin(alpha_n),  cos(alpha_n)]]
    Controlled: ctrl-V_n acts as V_n on ancilla when index=|0>,
                and as I on ancilla when index=|1>.

  Siphoning effect (per step):
    P(halt)     = sin²(alpha_n) * |t_n|²     (amplitude into ancilla=|1>)
    amp_continue = t_n * cos(alpha_n)         (remaining H_Good amplitude)
  ...
  Qiskit register layout (ancilla is qubit 0 = MSB):
    Ordering: anc[0], idx[0], content[0..m-1]
    Total qubits: 1 + 1 + m = m+2

  Controlled-V_n in Qiskit:
    RY(-2*alpha_n) gate on ancilla, controlled on idx=|0>.
    Implementation: X on idx; CRY(2*alpha_n) on (idx, anc); X on idx.
...
def build_controlled_Vn(alpha_n: float, m_content: int = 1) -> QuantumCircuit:
    ...
    Action: Apply RY(2*alpha_n) on the ancilla qubit, controlled by index=|0>.
    ...
    Implementation of ctrl-0 on idx:
        ...
        2. CRY(2*alpha_n) with idx as control, anc as target
    ...
    qc.cry(2.0 * alpha_n, idx[0], anc[0]) # CRY(2α): if idx=1, apply RY to anc
...
def build_foqa_sequence(
    ...
    """
    Build the full n_steps FOQA sequence circuit, including state initialization.

    State preparation:
        |0>_anc ⊗ [sin(theta)|0>_idx|0>_cont + cos(theta)|1>_idx|0>_cont]

    We encode H_Good via |0>_idx and H_Bad via |1>_idx. The content register
    is initialized to |0...0> (a proxy for the content state).

    alpha_n schedule:
        Mizel:   alpha_n = mizel_c / sqrt(n+1)
        Zeno:    alpha_n = zeno_alpha (constant, adversarial)

    Each step appends one ctrl-0 V_n gate (= 3 gates: X, CRY, X on anc+idx).
    The content register is not touched directly (in FOQA the oracle is
    implicit; the index qubit encodes which Hilbert space branch we are in).
    ...
    # --- State preparation ---
    # Prepare index qubit: RY(2*theta) produces sin(theta)|0> + cos(theta)|1>
    # The content register stays |0> (representing the target content state).
    qc.ry(2.0 * theta, 1)     # qubit 1 = idx[0]
...
def run_scenario_a(theta: float = 0.3, mizel_c: float = 1.5, m_content: int = 1) -> None:
    ...
    V_n = ctrl-0-RY(2*alpha_n) on the ancilla qubit, controlled by idx.
    Physical cost: X + CRY(2*alpha) + X = 2 CNOTs + parameterized single-qubit gates.
    """
    print("\n" + "=" * 70)
...
def run_scenario_c(theta: float = 0.01, zeno_alpha: float = 1.5, n_zeno_steps: int = 50) -> None:
    ...
    The state mathematically flatlines at p ~ sin²(theta) << 1, yet hardware
    must execute k * Depth(V_n) gates for mathematically zero gain.
    ...
    for k in step_checkpoints:
        ...
        qc_zeno.ry(2.0 * theta, 1)   # state prep on idx
    ...
    print(f"\n-> Mathematical result: p stays ~ sin²(theta) = {math.sin(theta)**2:.5f} (no amplification)")
    print("-> CONCLUSION: The Zeno schedule forces k * Depth(V_n) physical operations")
    print("   while achieving mathematically zero amplification. This is the literal")
    print("   definition of a coherence breach: depth explodes, benefit = 0.")
...
def run_scenario_f() -> None:
    ...
    We demonstrate that noise leaks amplitude into the |0>_anc state, producing
    ...
    print("=" * 70)
    # Use a very small theta (near 0) to simulate empty/near-empty database
    theta_empty = 1e-4   # sin²(theta) ~ 1e-8 ≈ 0 theoretically
    n_steps = 10
    mizel_c = 1.5
    print(f"theta={theta_empty:.1e} (sin²(theta)={math.sin(theta_empty)**2:.2e} ≈ 0)")
    ...
    p_theoretical = math.sin(theta_empty) ** 2 * (1 - (1 - math.sin(theta_empty)**2)**n_steps)

    print(f"{'Metric':<35} | {'Value':.6}")
    print("-" * 55)
    print(f"{'Theoretical success (ideal math)':<35} | {p_theoretical:.6e}")
    print(f"{'Measured success (ideal sim)':<35} | {p_ideal:.6f}")
    print(f"{'Measured success (1% noise)':<35} | {p_noisy:.6f}")
    print(f"{'False-positive leakage (noise - theory)':<35} | {abs(p_noisy - p_theoretical):.6f}")
    print("\n-> CONCLUSION: On an empty database, FOQA is mathematically safe (P_halt≈0).")
    print("   On physical hardware, depolarizing noise leaks amplitude into |1>_anc,")
    print("   creating a false-positive halt signal that destroys algorithmic guarantees.")
...
def run_scenario_g(theta: float = 0.3, n_steps: int = 5, mizel_c: float = 1.5) -> None:
    ...
    KEY FIX - Forced Fragmented Layout:
    We test the full n_steps FOQA sequence (with barriers) on a Linear 1D chain,
    ...
    for m in range(1, 6):
        ...
        qc.ry(2.0 * theta, 1)   # state prep on idx (logical qubit 1)
...
def run_scenario_h(theta: float = 0.3, mizel_c: float = 1.5) -> None:
    ...
    p = math.sin(theta) ** 2
    k_oaa = max(1, int(math.floor(math.pi / (4.0 * math.asin(math.sqrt(p))))))
    ...
    # Build OAA circuit (reuse from oaa_transpile primitives — inline here)
    def _build_A(p_):
        anc = QuantumRegister(1, "anc")
...
def run_scenario_i(theta: float = 0.3, mizel_c: float = 1.5) -> None:
    ...
    FOQA's fixed-point property means over-iterating (k >> k_opt) is
    mathematically safe — success probability stays high. But on NISQ hardware,
    every extra iteration adds depolarizing noise. We compare success at k_opt
    vs 3*k_opt to quantify the thermal decay cost of lazy halting.
    """
    try:
        from qiskit_aer import AerSimulator
        from qiskit_aer.noise import NoiseModel, depolarizing_error
    ...
    # Empirically find k_opt for FOQA by running the recurrence
    def foqa_recurrence(theta_, k, c):
        t_n = math.sin(theta_)
        s_n = math.cos(theta_)
        prob_halted = 0.0
        prob_cont = 1.0
        for n in range(k):
            ...
            t_n, s_n = t_new, s_new
        return prob_halted + prob_cont * (t_n**2)

    # Find k where theoretical success first exceeds 0.99
    k_opt_foqa = 1
    for k in range(1, 500):
        if foqa_recurrence(theta, k, mizel_c) >= 0.99:
            k_opt_foqa = k
            break
    ...
    for label, k in [("k_opt", k_opt_foqa), ("k_lazy (3x)", k_lazy)]:
        ...
        theory = foqa_recurrence(theta, k, mizel_c)
        results[label] = (k, d, cx, theory, p_succ)

    print(f"{'Schedule':<16} | {'k':<6} | {'Depth':<8} | {'CX':<6} | {'Theory P':<12} | {'Noisy P'}")
    print("-" * 70)
    for label, (k, d, cx, theory, p_noisy) in results.items():
        print(f"{label:<16} | {k:<6} | {d:<8} | {cx:<6} | {theory:<12.4f} | {p_noisy:.4f}")

    k0, d0, cx0, th0, pn0 = results["k_opt"]
    k1, d1, cx1, th1, pn1 = results["k_lazy (3x)"]
    ...
    print("-> CONCLUSION: FOQA is mathematically stable under over-iteration,")
    print("   but 3x iterations triples the NISQ noise exposure, severely degrading")
    print("   the physical success rate. An efficient halting heuristic is still mandatory.")
...
def run_scenario_k(theta: float = 0.3, n_steps: int = 30, mizel_c: float = 1.5) -> None:
    ...
    print(f"Exact schedule: {unique_exact} unique angles, P_success={p_exact:.6f}\n")
    print(f"{'Bins':<8} | {'Unique Angles':<16} | {'P_success':<12} | {'P degradation':<15} | {'Angle RMSE'}")
    print("-" * 70)
...
if __name__ == "__main__":
    ...
    output_filepath = os.path.join(script_dir, "!_FOQA_transpile_results.txt")
    ...
    try:
        run_scenario_a(theta=0.3, mizel_c=1.5, m_content=1)
        run_scenario_b(n_steps=10, theta=0.3, mizel_c=1.5)
        run_scenario_c(theta=0.01, zeno_alpha=1.5, n_zeno_steps=50)
        run_scenario_d(theta=0.3, n_steps=5, mizel_c=1.5)
        run_scenario_e(theta=0.3, mizel_c=1.5, m_content=3)
        run_scenario_f()
        run_scenario_g(theta=0.3, n_steps=5, mizel_c=1.5)
        run_scenario_h(theta=0.3, mizel_c=1.5)
        run_scenario_i(theta=0.3, mizel_c=1.5)
        run_scenario_j(theta=0.3, n_steps=5, mizel_c=1.5)
        run_scenario_k(theta=0.3, n_steps=30, mizel_c=1.5)
    except Exception:
        import traceback
        print("\n\n*** UNHANDLED EXCEPTION ***")
        traceback.print_exc()
