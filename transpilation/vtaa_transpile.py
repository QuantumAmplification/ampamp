...
_VTAA_PATH = os.path.join(_HERE, "5_Variable_Time_Amplitude_Amplification.py")
...
def run_scenario_a():
    ...
    print(SEP)
    print("Guessing M=1 when true M=5 in n=8 (N=256). Quantifying wasted depth.\n")

    res = vtaa.experiment_souffle_catastrophe(n=8, guessed_m=1, actual_m=5, k_scan_factor=1.5)
...
def run_scenario_b():
    ...
    res_ideal = vtaa.experiment_geometric_phase_staircase(n=8, k_max=20)

    # Now simulate noisy staircase via open-system (dephasing destroys phase linearity)
    try:
        res_noisy = vtaa.experiment_open_system_trajectory(n=4, k_max=12, phase_damp_1q=0.03, phase_damp_2q=0.10)
        has_noisy = True
    except Exception:
        has_noisy = False
...
def run_scenario_c():
    ...
    res = vtaa.experiment_2d_subspace_extractor(n=10, k_max=25, rank_threshold=1e-12)
    ...
    np.random.seed(42)
    N = 2 ** 10
    history_ideal, _, good_idx, _ = vtaa._standard_grover_state_history(n=10, k_max=25)
    # Add realistic noise: each state gets ~0.1% random perturbation
    history_noisy = [s + 0.001 * np.random.randn(N) for s in history_ideal]
    ...
    print(f"\nNoisy simulation (0.1% depolarizing perturbation per state):")
...
def run_scenario_d():
    ...
    res = vtaa.experiment_phase_mismatch_leakage(
        n=6, k_max=20, eps_oracle_deg=-5.0, eps_diff_deg=2.0,
        crosstalk_oracle_deg=0.6, local_z_detune_deg=0.6,
    )
...
def run_scenario_e():
    ...
    try:
        res = vtaa.experiment_open_system_trajectory(
            n=4, k_max=12, phase_damp_1q=0.02, phase_damp_2q=0.08,
        )
...
def run_scenario_f():
    ...
    res = vtaa.experiment_exact_amplitude_amplification(n=8, m_good=3, coarse_grid=48)
    ...
    print("-" * 95)
    for dac_bits in [6, 8, 10, 12, 14, 16]:
...
def run_scenario_g():
    ...
    res = vtaa.experiment_ftqc_diffusion_scaling(n_min=5, n_max=30, noancilla_max=8, optimization_level=1)
...
def run_scenario_h():
    ...
    res = vtaa.experiment_exact_amplitude_amplification(n=8, m_good=3)
    n = res.n
...
def run_scenario_k():
    ...
    res = vtaa.experiment_vtaa_cost_sweep(total_ps=0.05, t1=100.0, t2=1000.0, t3=10000.0)
    ...
    print(f"Total p_s = {res.total_ps:.2%}, stages: t1=100, t2=1000, t3=10000")
...
def run_scenario_l():
    ...
    t_halt = 100; t_max = 10000
    idle_time = t_max - t_halt  # in gate units
    cx_time_ns = 100  # ns per CX gate
    idle_ns = idle_time * cx_time_ns

    # T1, T2 typical values for superconducting qubits
    T1_ns = 100_000; T2_ns = 80_000
    n_qubits_idle = 4  # data register qubits sitting idle
...
def run_scenario_m():
    ...
    zz_rate_per_cx = 0.002  # 0.2% ZZ phase per CX gate on neighbor
    n_data_cx_gates = 500   # typical deep oracle on data register
    n_flag_qubits = 3       # (2 flags + 1 clock)
    ...
    print("-" * 55)
    for cx in [100, 250, 500, 1000, 2000]:
...
def run_scenario_n():
    ...
    mid_meas_latency_ns = 1000  # 1 μs per mid-circuit measurement
    cx_time_ns = 100
...
if __name__ == "__main__":
    ...
    output_filepath = os.path.join(script_dir, "!_VTAA_transpile_results.txt")
    ...
    print(f"\n{SEP}")
    print("Benchmark suite complete. 15 scenarios executed.")
    logger.close()
