...
class HardwareProfiler:
    ...
    def profile_circuit(self, logical_circuit: QuantumCircuit) -> Dict[str, Any]:
        ...
        # Stage 0: Init
        # We must unroll massive multi-qubit gates (like mcp) to 2Q and 1Q basis gates
        # so that SabreLayout doesn't crash internally expecting simple edges.
        ...
        # Stage 2: Routing
        qc_routing = pm.routing.run(qc_layout)
        metrics['post_routing_depth'] = qc_routing.depth()
        # SWAP count is basically everything added in routing that is a swap
        routing_ops = qc_routing.count_ops()
        metrics['routing_swaps'] = routing_ops.get('swap', 0)
        
        # Stage 3: Translation
        qc_translation = pm.translation.run(qc_routing)
        trans_ops = qc_translation.count_ops()
        metrics['post_translation_depth'] = qc_translation.depth()
        metrics['translation_gates'] = sum(trans_ops.values())
        metrics['translation_cnots'] = trans_ops.get('cx', 0)
        
        # Stage 4: Optimization
        qc_opt = pm.optimization.run(qc_translation)
