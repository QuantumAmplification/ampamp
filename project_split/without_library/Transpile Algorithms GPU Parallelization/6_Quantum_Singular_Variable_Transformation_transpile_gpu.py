...
def _format_signature_help(fn: Any) -> str:
...
def _interactive_scenario_repl(scenarios: List[Tuple[str, Any]]) -> None:
    if not sys.stdin.isatty():
        return

    scenario_map = {label.upper(): fn for label, fn in scenarios}
    print(f"\n{SEP}")
    print("INTERACTIVE SCENARIO RE-RUN MODE")
    print(SEP)
    print("You can now rerun any scenario with custom inputs.")
    print("Enter a scenario letter like A or Z, or press Enter to finish.")

    while True:
        try:
            choice = input("\nScenario letter to rerun: ").strip().upper()
        except EOFError:
        ...
        print("Enter overrides as comma-separated key=value pairs, for example:")
        print("  n_data=128, degree=41")
        print("Press Enter to use defaults.")

        try:
            raw_kwargs = input("Custom parameters: ")
            kwargs = _parse_kwargs_text(raw_kwargs)
            print(f"\nRe-running scenario {choice} with parameters: {kwargs if kwargs else 'defaults'}")
            fn(**kwargs)
...
STANDARD_QUBITS = 20
_QSVT_PATH = os.fspath(
    resolve_project_file(__file__, "6_Quantum_Singular_Variable_Transformation.py", preferred_dirs=("Theory Algorithms",))
)
_FPAA_PATH = os.fspath(
    resolve_project_file(__file__, "2_Fixed_Point_Ammplitude_Amplification.py", preferred_dirs=("Theory Algorithms",))
)
_PROF_PATH = os.fspath(
    resolve_project_file(__file__, "quantum_profiler_gpu.py", preferred_dirs=("Transpile Algorithms Parallelization",))
)
...
def _min_heavy_hex_distance(n_qubits: int) -> int:
    distance = 1
    while CouplingMap.from_heavy_hex(distance=distance).size() < n_qubits:
        distance += 2
    return distance


def _heavy_hex_map(min_qubits: int = 1) -> CouplingMap:
    return CouplingMap.from_heavy_hex(distance=_min_heavy_hex_distance(min_qubits))
...
def build_foqa_benchmark_circuit(n_data: int = 4, rounds: int = 3) -> QuantumCircuit:
    ...
    qc.h(data)
    qc.ry(0.6, idx)
    schedule = np.linspace(1.2, 0.6, num=max(1, rounds))
...
def build_svd_channel_skeleton(n_qubits: int = 2) -> QuantumCircuit:
    if n_qubits < 2:
        raise ValueError("build_svd_channel_skeleton requires at least 2 qubits.")
    # Minimal toy channel on the first 2 qubits; extra qubits are idle spectators.
...
def run_scenario_a(n_data: int = 6) -> None:
    print(f"\n{SEP}")
    ...
    qc_qsvt = build_qsvt_lifted_circuit(n_data=n_data, phases=phases, include_work=True)

    hh = _heavy_hex_map(min_qubits=qc_qsvt.num_qubits)
...
def run_scenario_b(n_data: int = 8, phase_count: int = 19) -> None:
    print(f"\n{SEP}")
    print("SCENARIO B: SINGLE-SIGNAL ROUTING BOTTLENECK (HEAVY-HEX ROUTING CONGESTION)")
    print(SEP)

    phases = np.linspace(-1.1, 1.1, phase_count)
    qc = build_qsvt_lifted_circuit(n_data=n_data, phases=phases, include_work=True, extra_route=True)
    hh = _heavy_hex_map(min_qubits=qc.num_qubits)
...
def run_scenario_c(n_data: int = 5, phase: float = 0.31) -> None:
    print(f"\n{SEP}")
    print("SCENARIO C: ALTERNATING UNCOMPUTATION LIMIT (U - PHASE - U^dagger)")
    print(SEP)

    qc_wall = build_u_wall_circuit(n_data=n_data, phase=phase, inject_phase=True)
    qc_no_wall = build_u_wall_circuit(n_data=n_data, phase=phase, inject_phase=False)
    hh = _heavy_hex_map(min_qubits=max(qc_wall.num_qubits, qc_no_wall.num_qubits))
...
def run_scenario_d(n_data: int = 5) -> None:
    print(f"\n{SEP}")
    ...
    qc = build_lcu_prep_sel_circuit(n_data=n_data)
...
def run_scenario_f(n_data: int = 4) -> None:
    print(f"\n{SEP}")
    ...
    print("-" * 58)
    for d in degrees:
        qc = build_qsvt_degree_circuit(n_data=n_data, degree=int(d))
...
def run_scenario_g(n_data: int = 3, qsvt_degree: int = 41) -> None:
    print(f"\n{SEP}")
    ...
    qc_qsvt = build_qsvt_degree_circuit(n_data=n_data, degree=qsvt_degree)
    qc_qpe = build_qpe_hhl_skeleton(n_data=n_data, n_phase=qpe_anc)
...
def run_scenario_h(n_data: int = 5, degrees: Tuple[int, ...] = (9, 21, 41, 61), coherence_ns: float = 80_000.0) -> None:
    print(f"\n{SEP}")
    ...
    breach_degree = None
    for d in degrees:
        qc = build_qsvt_degree_circuit(n_data=n_data, degree=d)
        line = CouplingMap(_line_edges(qc.num_qubits))
...
def run_scenario_i(n_qubits: int = 4, max_layers: int = 6) -> None:
    print(f"\n{SEP}")
...
def run_scenario_j(n_data: int = 4, core_degree: int = 15) -> None:
    print(f"\n{SEP}")
    ...
    qc_core = build_qsvt_degree_circuit(n_data=n_data, degree=core_degree)
...
def run_scenario_k(n_data: int = 5, degrees: Tuple[int, ...] = (5, 15, 25, 35, 45), coherence_ns: float = 80_000.0) -> None:
    print(f"\n{SEP}")
...
def run_scenario_n(n_data: int = 4, rounds: int = 5) -> None:
    print(f"\n{SEP}")
    ...
    strict_qc = build_strict_parity_circuit(n_data=n_data, rounds=rounds)
    mixed_qc = build_mixed_parity_circuit(n_data=n_data, rounds=rounds)
...
def run_scenario_o(n_data: int = 4, degree: int = 41) -> None:
    print(f"\n{SEP}")
    ...
    qc = build_qsvt_degree_circuit(n_data=n_data, degree=degree)
    prof = _profile_stats(qc, CouplingMap(_line_edges(qc.num_qubits)))
...
def run_scenario_q(test_bits: Tuple[int, ...] = (4, 8, 12)) -> None:
    print(f"\n{SEP}")
    ...
    res = qsvt_mod.experiment_adversarial_phase_quantization(
        degree=35,
        test_bits=test_bits,
        sweep_range=(min(test_bits), max(test_bits)),
        num_points=1001,
        plot=False,
    )
...
def run_scenario_s(n_data: int = 5, degree: int = 21) -> None:
    print(f"\n{SEP}")
...
def run_scenario_t(
    n_data: int = STANDARD_QUBITS,
    grover_iterations: int = 3,
    fpaa_length: int = 3,
    dqaa_prefix_bits: int = 1,
    qsvt_degree: int = 21,
) -> None:
    print(f"\n{SEP}")
    ...
    circuits: Dict[str, QuantumCircuit] = {
        ...
        "DQAA": build_dqaa_benchmark_circuit(n_global=n_data, j=dqaa_prefix_bits, rounds=2),
        "VTAA": build_vtaa_benchmark_circuit(n_data=n_data, rounds=2),
        "QSVT": build_qsvt_degree_circuit(n_data=n_data, degree=qsvt_degree),
    }

    hh = _heavy_hex_map(min_qubits=max(qc.num_qubits for qc in circuits.values()))
...
def run_scenario_u(n_data: int = STANDARD_QUBITS, rounds: int = 6) -> None:
    print(f"\n{SEP}")
    print("SCENARIO U: DYNAMIC QUBIT REUSE (MID-CIRCUIT RESET TRADEOFF)")
    print(SEP)

    dyn = build_dynamic_reuse_qsvt_circuit(n_data=n_data, rounds=rounds)
    static = build_static_signal_bank_qsvt_circuit(n_data=n_data, rounds=rounds)
...
def run_scenario_v(n_data: int = STANDARD_QUBITS, degree: int = 25) -> None:
    print(f"\n{SEP}")
    print("SCENARIO V: RANDOMIZED COMPILING (PAULI TWIRLING OVERHEAD)")
    print(SEP)

    base = build_qsvt_degree_circuit(n_data=n_data, degree=degree)
    twirled = build_twirled_circuit(base, seed=2026)
    hh = _heavy_hex_map(min_qubits=max(base.num_qubits, twirled.num_qubits))
...
def run_scenario_w(n_data: int = STANDARD_QUBITS, qsvt_degree: int = 17, fpaa_length: int = 9) -> None:
    print(f"\n{SEP}")
    print("SCENARIO W: MAGIC STATE FACTORY FOOTPRINT (15-TO-1 CAPACITY)")
    print(SEP)

    qsvt_c = build_qsvt_degree_circuit(n_data=n_data, degree=qsvt_degree)
    fpaa_c = fpaa_mod.build_fpaa_circuit(num_qubits=n_data, L=fpaa_length, delta=0.1, good_indices=[0])
    hh = _heavy_hex_map(min_qubits=max(qsvt_c.num_qubits, fpaa_c.num_qubits))
...
def run_scenario_x(n_data: int = STANDARD_QUBITS, degree: int = 21) -> None:
    print(f"\n{SEP}")
    ...
    base_core = build_qsvt_degree_circuit(n_data=n_data, degree=degree)
...
def run_scenario_y(n_data: int = STANDARD_QUBITS) -> None:
    print(f"\n{SEP}")
    print("SCENARIO Y: BLOCK-ENCODING TRANSLATION TRADEOFF (LCU vs TROTTER)")
    print(SEP)

    phases = np.sin(np.linspace(0.0, 2.0 * np.pi, 17)) * 0.9
    lcu = build_lcu_embedding_qsvt_circuit(phases=phases, n_data=n_data)
    trotter = build_trotter_embedding_qsvt_circuit(phases=phases, n_data=n_data)
    hh = _heavy_hex_map(min_qubits=max(lcu.num_qubits, trotter.num_qubits))
...
def run_scenario_z(
    ...
    good = [0]
    circuits: Dict[str, QuantumCircuit] = {
        "Grover": fpaa_mod.build_standard_grover_circuit(num_qubits=n_data, iterations=grover_iterations, good_indices=good),
        "FPAA": fpaa_mod.build_fpaa_circuit(num_qubits=n_data, L=fpaa_length, delta=0.1, good_indices=good),
        "OAA": build_oaa_benchmark_circuit(n_data=n_data, rounds=2),
        "FOQA": build_foqa_benchmark_circuit(n_data=n_data, rounds=3),
        "DQAA": build_dqaa_benchmark_circuit(n_global=n_data, j=dqaa_prefix_bits, rounds=2),
        "VTAA": build_vtaa_benchmark_circuit(n_data=n_data, rounds=2),
...
def _save_qsvt_algorithm_figure(
    n_data: int = 5,
    *,
    degrees=(3, 9, 17, 25),
    output_name="qsvt_hardware_tradeoff_profile.png",
):
    plt = _load_pyplot()
    qsp_depth = []
    qsvt_depth = []
    ...
    for degree in degrees:
        phases = np.linspace(-1.0, 1.0, int(degree) + 1)
        qc_qsp = build_qsp_signal_circuit(phases)
        qc_qsvt = build_qsvt_lifted_circuit(n_data=n_data, phases=phases, include_work=True)
        hh = _heavy_hex_map(min_qubits=qc_qsvt.num_qubits)
        m_qsp = _transpile_stats(qc_qsp, coupling_map=hh, optimization_level=3)
        m_qsvt = _transpile_stats(qc_qsvt, coupling_map=hh, optimization_level=3)
        qsp_depth.append(float(m_qsp["depth"]))
        qsvt_depth.append(float(m_qsvt["depth"]))
        qsp_cx.append(float(m_qsp["cx"]))
        qsvt_cx.append(float(m_qsvt["cx"]))
        qsvt_qubits.append(float(m_qsvt["qubits"]))

    fragility = qsvt_mod.experiment_physical_phase_fragility(
        degree=15,
        phase_error=0.05,
        max_depth=20,
        plot=False,
    ...
    fig.suptitle("QSVT Hardware Tradeoff Profile", fontsize=14, fontweight="bold")
    ax1, ax2, ax3, ax4 = axes.flat

    ax1.plot(degrees, qsp_depth, marker="o", linewidth=2.0, label="QSP signal model", color="#1f77b4")
    ax1.plot(degrees, qsvt_depth, marker="s", linewidth=2.0, label="lifted QSVT", color="#d62728")
    ax1.set_title("Depth vs Polynomial Degree")
    ax1.set_ylabel("Depth")
    ax1.legend(fontsize=8)

    ax2.plot(degrees, qsp_cx, marker="o", linewidth=2.0, label="QSP signal model", color="#2ca02c")
    ax2.plot(degrees, qsvt_cx, marker="s", linewidth=2.0, label="lifted QSVT", color="#9467bd")
    ax2.set_title("Entangling Cost vs Degree")
    ax2.set_ylabel("CX count")
    ax2.legend(fontsize=8)

    ax3.plot(degrees, qsvt_qubits, marker="D", linewidth=2.2, color="#ff7f0e")
    ax3.set_title("Lifted Width Requirement")
    ax3.set_ylabel("Physical qubits")

    ax4.plot(fragility.depths, fragility.fidelity_decay, linewidth=2.2, color="#8c564b")
    ax4.set_title("Phase Fragility")
    ax4.set_ylabel("Fidelity decay")

    ax1.set_xlabel("Polynomial degree")
    ax2.set_xlabel("Polynomial degree")
    ax3.set_xlabel("Polynomial degree")
    ax4.set_xlabel("Depth index")
    ...
    save_figure_with_metadata(
        ...
        output_path,
        {
            "figure_kind": "qsvt_hardware_tradeoff_profile",
            "n_data": int(n_data),
            ...
            "phase_grid_formula": "linspace(-1.0, 1.0, degree + 1)",
        },
    )
    plt.close(fig)
...
def _save_qsvt_embedding_figure(
    n_data: int = 6,
    *,
    output_name="qsvt_embedding_tradeoff.png",
):
    plt = _load_pyplot()
    phases = np.sin(np.linspace(0.0, 2.0 * np.pi, 17)) * 0.9
    lcu = build_lcu_embedding_qsvt_circuit(phases=phases, n_data=n_data)
    ...
    labels = ["LCU", "Trotter"]
    qubits = [float(lcu.num_qubits), float(trotter.num_qubits)]
    swaps = [float(m_lcu.get("routing_swaps", 0)), float(m_trot.get("routing_swaps", 0))]
    cnots = [float(m_lcu["final_cnots"]), float(m_trot["final_cnots"])]
    times = [float(m_lcu["total_time_ns"]), float(m_trot["total_time_ns"])]

    x = np.arange(len(labels), dtype=float)
    fig, axes = plt.subplots(2, 2, figsize=(13.5, 9.5), constrained_layout=True)
    fig.suptitle("QSVT Embedding Tradeoff", fontsize=14, fontweight="bold")
    ax1, ax2, ax3, ax4 = axes.flat

    ax1.bar(x, qubits, color="#1f77b4")
    ax1.set_title("Physical Qubits")
    ax1.set_ylabel("Qubits")

    ax2.bar(x, swaps, color="#d62728")
    ax2.set_title("Routing SWAPs")
    ax2.set_ylabel("SWAP count")

    ax3.bar(x, cnots, color="#2ca02c")
    ax3.set_title("Final CNOT Count")
    ax3.set_ylabel("CNOTs")

    ax4.bar(x, times, color="#9467bd")
    ax4.set_title("Scheduled Time")
    ...
    save_figure_with_metadata(
        ...
        output_path,
        {
            "figure_kind": "qsvt_embedding_tradeoff",
            "n_data": int(n_data),
            "embedding_labels": list(labels),
            "phase_count": int(len(phases)),
            "phase_formula": "0.9 * sin(linspace(0, 2*pi, 17))",
...
if __name__ == "__main__":
    ...
    output_path = os.path.join(_RESULT_DIR, "terminal_output.log")
    ...
    scenarios = wrap_scenarios(
        raw_scenarios,
        module_globals=globals(),
        extra_patch_objects=(qsvt_mod, fpaa_mod),
    ...
    if not cli_executed:
        ...
        _interactive_scenario_repl(scenarios)

    print(f"\n{SEP}")
    print(f"Benchmark suite complete. {'1 scenario executed via direct CLI.' if cli_executed else '26 scenarios executed.'}")
    render_backend_validation_summary(publishability)
    _save_qsvt_algorithm_figure()
    _save_qsvt_embedding_figure()
    logger.close()
