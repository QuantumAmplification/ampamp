...
def _gpu_statevector_data(qc: QuantumCircuit, *, seed: int = 7) -> np.ndarray:
    try:
...
def _single_target_success(instance, circuit: QuantumCircuit, controlled: bool) -> float:
    state = _gpu_statevector_data(circuit)
    if controlled:
...
def run_scenario_g(epsilon: float = 0.10, repetitions: int = 2, topology: str = "line") -> None:
    ...
    structured_state = _gpu_statevector_data(structured)
    direct_state = _gpu_statevector_data(direct)
