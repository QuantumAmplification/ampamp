...
class QAOAGroverLab:
    ...
    def exact_circuit_state(self, params: Sequence[float], mixer_type: str) -> np.ndarray:
        return np.asarray(Statevector.from_instruction(self.build_qaoa_circuit(params, mixer_type)).data, dtype=complex)
    ...
    def sample_success_probability(
        ...
        *,
        ...
        try:
            ...
            raise RuntimeError("qiskit-aer is required for QAOA backend sampling.") from exc
    ...
    def save_summary(self, results: QAOAGroverResults, filename: Optional[str] = None) -> str:
        ...
        with open(output_path, "w", encoding="utf-8") as handle:
            handle.write("MODULE 1.1: QAOA-INSPIRED SEARCH COMPARISON\n")
            handle.write("=" * 64 + "\n")
...
def run_interactive_scenario_repl(scenarios) -> None:
    ...
    print("\n" + "=" * 72)
    print("INTERACTIVE CASE RE-RUN MODE")
    print("=" * 72)
    print("You can now rerun any case with custom inputs.")
    ...
    while True:
        ...
        try:
            ...
            print(f"\nCould not parse or run custom parameters for case {choice}.")
            print(f"Error: {exc}")
...
def main() -> None:
    ...
    try:
        print("-" * 65)
        print("MODULE 1.1: QAOA-INSPIRED SEARCH COMPARISON")
        print("-" * 65)
        ...
        if sys.stdin.isatty():
            print("\nDefault suite complete.")
            print("Dense Case F is heavier, so it is left opt-in via interactive rerun.")
            print("You can now choose any case and enter custom parameters.")
