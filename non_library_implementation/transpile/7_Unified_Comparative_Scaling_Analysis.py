...
from transpile_path_utils import ensure_directory_on_syspath
...
def _transpile_stats(qc: QuantumCircuit, backend: AerSimulator, seed: int = 42) -> Dict[str, Any]:
    tqc = transpile(
        qc,
        backend=backend,
...
def run_scenario_a(n_data: int = 4, rounds: int = 2, seed: int = 42) -> None:
    ...
    print(SEP)
    print(f"Representative circuits compiled with AerSimulator for n_data={n_data}, rounds={rounds}.\n")
