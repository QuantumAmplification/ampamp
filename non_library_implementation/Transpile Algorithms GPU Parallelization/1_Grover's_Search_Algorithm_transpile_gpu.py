...
def run_scenario_e(n_qubits=4, max_k=10):
    ...
    try:
        ...
        print("Missing or incompatible qiskit_aer install. Cannot run noise simulation.")
...
def run_profiling_benchmark(n_qubits=STANDARD_QUBITS):
    ...
    try:
        profiler_mod = _import_local_module("grover_quantum_profiler_module", "quantum_profiler_gpu.py")
        HardwareProfiler = profiler_mod.HardwareProfiler
    except Exception:
        print("Could not import HardwareProfiler. Ensure quantum_profiler_gpu.py is available.")
        return
